...
class CodaDataset(Dataset):
    def __init__(self, hf_dataset, exist_ans = True):
        ...
        self.filtered_indices = [
            ...
            if hf_dataset[i]["id"].split("_")[1] in ["suggestion"]
        ]
...
class LlavaModelPLModule(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if torch.isnan(pixel_values).any():
            print("NaN found in pixel_values")
        if torch.isnan(input_ids.float()).any():
            print("NaN found in input_ids")
        ...
        # Check each component of the output
        if hasattr(outputs, 'logits') and torch.isnan(outputs.logits).any():
            print("NaN found in logits")
            
        if hasattr(outputs, 'last_hidden_state') and torch.isnan(outputs.last_hidden_state).any():
            print("NaN found in last_hidden_state")

        loss = outputs.loss
        if torch.isnan(loss):
            print(f"NaN loss encountered in batch {batch_idx}")
            print(f"Loss value: {loss}")
            print(f"Input shape: {input_ids.shape}")
            print(f"Max input value: {torch.max(input_ids)}")
            print(f"Image shape: {pixel_values.shape}")
            print(f"Image max/min: {torch.max(pixel_values)}, {torch.min(pixel_values)}")
            return None
...
class PushToHubCallback(Callback):
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        output_dir = f"{OUTPUT_DIR}/llava-fintune-epoch-suggestion-{trainer.current_epoch}"

...
with open('submission_first.json') as f:
    first_stage = json.load(f)
# Constants
BASE_MODEL_ID = "llava-hf/llava-1.5-7b-hf"
ADAPTER_PATH = "model/reviewer-epoch-0"
MAX_LENGTH = 750
...
with torch.no_grad():
    for sample_ids, tasks, images, human_msgs in tqdm(test_loader):
        ...
        if tasks[0] == "general":
            ...
            human_msg = (
                f"<image>\n Here are the key elements in the image: {ram_pred[sample_key]}. INITIAL DESCRIPTION: {first_stage[sample_id]}"
                ...
                "car's driving behavior."
                )
        elif tasks[0] == "regional":
            human_msg = (
                "<image>\nPlease describe the object inside the red rectangle in the image. "
                "Describe its color, position, status, implication, response, and explain why "
                f"it affects ego car driving. ASSISTANT (Draft Version): {first_stage[sample_id]}\n "
                "Review Task: This is the draft version of the inference generated by our system. "
                "Please carefully review the image again, describe the object inside the red rectangle "
                "in the image. Describe its color, position, status, implication, response, and explain "
                "why it affects ego car driving. Identify any inaccuracies, missing details, or incorrect "
                "interpretations in the draft version. Provide a corrected and comprehensive final answer "
                "that accurately describes the object inside the red rectangle in the image."
            )
        # human_msg = human_msg + "Please read the following message again: "+ human_msg
        elif tasks[0] == "suggestion":
            sample_key = "Test_general_"+ sample_id.split("_")[2]
            human_msg = (
                f"<image>\nThis is an image of traffic captured from the perspective of the ego car. "
                f"Here are the key elements in the image: {ram_pred[sample_key]}. Focus on objects in the "
                ...
                "You must not discuss any objects beyond the seven categories above. For each identified object, "
                "provide actionable driving suggestions for the ego car, considering the object's position, status, "
                "and implications. Recommendations should prioritize safety, legal compliance, and efficient driving "
                f"behavior. ASSISTANT (Draft Version): {first_stage[sample_id]}\n Review Task: This is the draft version "
                "of the inference generated by our system. Please carefully review the image again, focusing on the seven "
                "categories mentioned earlier (vehicles, vulnerable road users, traffic signs, traffic lights, traffic cones, "
                "barriers, and miscellaneous). Identify any inaccuracies, missing details, or incorrect interpretations in the "
                "draft version. Provide a corrected and comprehensive final answer that accurately provides actionable driving "
                "suggestions for the ego car, considering the object's position, status, and implications. Recommendations should "
                "prioritize safety, legal compliance, and efficient driving behavior."
            )
        prompts = [prompt_template.format(human_msg + 'You should directly and accurately answer the question.')]
        ...
        with open("submission_general.json", "w") as f:

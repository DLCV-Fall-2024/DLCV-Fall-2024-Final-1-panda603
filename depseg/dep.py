...
WANDB_NAME = "lora_and_dep"
OUTPUT_DIR = "model_all"
DATA_ROOT = ""
...
class CustomLlavaModel(LlavaForConditionalGeneration):
    ...
    def __init__(self, config):
        ...
        self.depth_alpha = nn.Parameter(torch.zeros(1)) #請打開
    ...
    def get_depth_features(self, images):
        ...
        depth_map = depth_map.squeeze(1) # [batch_size, image_size, image_size]
        ...
        depth_features = self.depth_projection(patches)  # [B, 576, 4096]
...
def find_all_linear_names(model):
    ...
    multimodal_keywords = ["multi_modal_projector", "vision_model","depth_projection","depth_model"]
...
# 打開 depth 相關參數的訓練
model.depth_alpha.requires_grad = True
model.depth_projection.requires_grad = True
...
def eval_collate_fn(examples):
    ...
    for example in examples:
        ...
        texts.append(prompt)
        answers.append(assistant_msg+'</s>')
...
class CustomLlavaModelPLModule(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # 2.5 Get depth features, Combine features
        depth_features = self.model.get_depth_features(original_images)
        combined_features = projected_vision_features + self.model.depth_alpha * depth_features
        ...
        self.log("dep_alpha", self.model.depth_alpha)
        return loss
...
class PushToHubCallback(Callback):
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        depth_state = {
            'depth_alpha': pl_module.model.depth_alpha.data,
            'depth_projection.weight': pl_module.model.depth_projection.weight.data,
            'depth_projection.bias': pl_module.model.depth_projection.bias.data
        }
        torch.save(depth_state, os.path.join(output_dir, 'depth_params.bin'))
        print(f"Saved depth_alpha value: {pl_module.model.depth_alpha.item()}")
...
model.depth_alpha.requires_grad = True
model.depth_projection.requires_grad = True
print(f"depth_alpha requires_grad: {model.depth_alpha.requires_grad}")
print(f"depth_alpha value: {model.depth_alpha.item()}")
trainer.fit(model_module)